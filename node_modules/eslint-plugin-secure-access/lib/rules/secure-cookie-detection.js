    ...
    return {
      CallExpression(node) {
        if (
          ...
          const options =
            optionsIndex !== -1 &&
            node.arguments[optionsIndex].type === "ObjectExpression"
          ...
          if (options) {
            options.properties.forEach((property) => {
              if (
                ...
                (property.key.name === "Expires" ||
                  property.key.name === "maxAge")
              ) {
            ...
            context.report({
              ...
              fix(fixer) {
                ...
                if (!options) {
                  ...
                  if (!hasHttpOnly) {
                    fixes.push(
                      fixer.insertTextAfter(lastOption, ", httpOnly: true")
                    );
                  }

                  if (!hasSameSite) {
                    fixes.push(
                      fixer.insertTextAfter(lastOption, ', sameSite: "Strict"')
                    );
                  }
                }
