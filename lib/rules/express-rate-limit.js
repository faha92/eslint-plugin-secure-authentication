module.exports = {
  create: function (context) {
    ...
    return {
      ...
      CallExpression(node) {
        if (!hasLimiterUsage && checkAppUseForLimiter(node)) {
          hasLimiterUsage = true;
          if (!expressRateLimitImported) {
            context.report({
              ...
              fix: function (fixer) {
                ...
                const limiterUsageFix = fixer.insertTextBefore(node.arguments[node.arguments.length - 1], 'limiter, ');

                return [importFix, limiterUsageFix];
              },
            });
          }
        }
