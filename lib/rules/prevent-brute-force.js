module.exports = {
  ...
  if (
    ...
    /\/login/i.test(node.arguments[0].value)
  ) {
        ...
        if (!hasExpressBruteImport && loginRouteNode) {
          context.report({
            ...
            fix(fixer) {
              ...
              const preventMiddlewareText = "const bruteforce = new ExpressBrute(store, {\n  freeRetries: 5,\n  minWait: 6 * 1000, // <-- set to 60 seconds per default \n  failCallback: function (req, res, next, nextValidRequestDate) {\n    console.log('Too many failed login attempts. Rate limiting request.');\n    req.flash(\n      'error',\n      'Too many failed login attempts. Please try again later.'\n    );\n    res.redirect('/users/login');\n  },\n});\n\n";

              const insertImportFix = fixer.insertTextBeforeRange(context.getSourceCode().ast.range, importText + preventMiddlewareText);
