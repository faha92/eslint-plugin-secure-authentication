      ...
      CallExpression(node) {
        if (
          ...
          if (variable && variable.defs[0].node.init.type === "NewExpression") {
            const properties = variable.defs[0].node.init.arguments[0].properties;
                ...
                context.report({
                  ...
                  fix(fixer) {
                    ...
                    if (!bcryptImported) {
                      ...
                      bcryptImported = true;
                      return [
                        fixer.insertTextBeforeRange([objectStart, objectStart], `const bcrypt = require("bcryptjs");\n`),
                        fixer.insertTextBeforeRange([objectStart, objectStart], `const hashedPassword = bcrypt.hashSync(${property.value.name}, 10);\n`)
                      ];
                    }
